...
%Het bepalen van de linker- en rechterrand van de primitieven langs de as, vereist scalaire producten in tegenstelling tot bij de $\symKd$ boom.
%Het splitsen van de omhullende volumes en het bereken van de oppervlaktes is complexer omdat met \symKDOP{}s gewerkt moet worden in plaats van met asgealigneerde balken.
...
\label{sec:h4-bspize}
% TODO Vergelijk representatie met die van de paper
...
\begin{dutchalgorithm}
    \begin{algorithmic}       
        ...
        \EndFunction
    \end{algorithmic}
    \caption{Beste split voor een bouwknoop b bij een $\symBSPizefastkd$ boom.}
    ...
    \begin{algorithmic}       
        \Function{bepaal\_richtingen}{b, k}
            ...
            \For{$i \in \{0,1,2,...,k-1\}$}
                    ...
                    \State $richting \gets $ \Call{update\_zin}{richting}
                    \State \Call{add}{richtingen, richting}
            \EndFor
    ...
    \begin{algorithmic}       
        \Function{bepaal\_richtingen}{b, k}
            ...
            \While{$richtingen_{lengte} < $\Call{min}{$k-1$,$b_n - 1$}}
                    \State $richting \gets b_{primitieven}[$\Call{random\_int}{0, $b_n$}$]_{normaal}$
                    \State $richting \gets $ \Call{update\_zin}{richting}
                    \State \Call{add}{richtingen, richting}
            \EndWhile
            \State \Return $richtingen$
...
\begin{dutchalgorithm}
    \begin{algorithmic}       
        \Function{bepaal\_richtingen}{b, k}
            \State $\forall i \in \{0,1,...,b_n-1\}: normalen[i] \gets $\Call{update\_zin}{$b_{primitieven}[i]_{normaal}$}
            \If{$b_n \leq k$}
                \State \Return $normalen$
            \EndIf
            \State $clusterCentra \gets \emptyset$
            \While{$clusterCentra_{lengte} < k$}
                \State $clusterCentrum \gets normalen[$\Call{random\_int}{0, $b_n$}$]$
                \State \Call{add}{clusterCentra, clusterCentrum}
            \EndWhile
            ...
            \While{$iteratie < maxIteraties$ \And ($iteraties = 0$ \Or $oudeClusterCentra \neq clusterCentra$ )}
                ...
                \For{$i \in \{0,1,2..k-1\}$}
                    \If{$clusters[i] = \emptyset$}
                        ...
                        \EndWhile
                    \EndIf
                    \State $clusterCentra[i] \gets $ \Call{gemiddelde}{$clusters[i]$}
                \EndFor
...
\begin{dutchalgorithm}
    \begin{algorithmic}       
        ...
        \EndFunction
    \end{algorithmic}
    \caption{Beste split voor een bouwknoop b bij een $\symBSPsweepkd$  boom.}
